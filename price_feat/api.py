...
@frappe.whitelist()
def fetch_latest_customer_price(doctype,item_code,docname):
    if doctype=='Sales Invoice':
        return frappe.db.sql("""SELECT child_item.rate  from `tabSales Invoice` as main inner join `tabSales Invoice Item` as child_item 
                ...
                and main.docstatus != 2 and main.name!=%s order by main.posting_date  Desc limit 1""",(item_code,docname),as_dict=1,debug=1)
    elif doctype=='Sales Order':
        return frappe.db.sql("""SELECT child_item.rate  from `tabSales Order` as main inner join `tabSales Order Item` as child_item 
                on main.name=child_item.parent where child_item.item_code = %s 
                and main.docstatus != 2 and main.name!=%s order by main.transaction_date  Desc limit 1""",(item_code,docname),as_dict=1,debug=1)
